...
export default function Header() {
  ...
  export default function Header() {
    ...
    // Tilstand for om overlayet er synlig
    const [isCartOverlayVisible, setCartOverlayVisible] = useState(false);
  
    // Refs for overlay og ikon-container
    const overlayRef = useRef(null);
    const cartIconContainerRef = useRef(null);
  
    // Veksle overlay ved klikk på ikonet
    const toggleCartOverlay = () => {
      setCartOverlayVisible((prev) => !prev);
    };
  
    // Lukk overlay ved klikk utenfor overlayet
    useEffect(() => {
      const handleClickOutside = (event) => {
        // Hvis overlayet er synlig og klikket ikke er inne i overlay eller ikon-containeren, lukk overlayet
        if (
          isCartOverlayVisible &&
          overlayRef.current &&
